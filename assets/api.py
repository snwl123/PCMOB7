...
class User(db.Model):
    ...
    events = db.relationship('Events', backref='user', cascade="delete, save-update")
...
@app.route('/user/<int:user_id>/events', methods=["POST", "GET"])
def allEvents(user_id):
    if request.method == "GET":
        events = Events.query.filter_by(user_id=user_id)
        json_events = []
        for event in events:
...
def doRollback():
    db.session.rollback()
    return { "Message": "Completed rollback."}
